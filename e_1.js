...
function addressFormally(fullName, isMarried, isFemale) {
    ...
    let name = ''
    let nameSplit = fullName.split(' ') // this is an array [firstName, Last Name] // split by spaces between words
    name = nameSplit[0]
    
    let lastName = nameSplit[1]
    let firstCharacterOfLastName = lastName[0] // or only lastName[0][1]
...
function averageAge(ages) { 
    let sum = 0
    let negativeCount = 0
    for (age of ages) { // ((name of variable) of (array))
      }  if (age >= 0) {
        sum += age
        } else {
            negativeCount++
        }
    return sum / (ages.length - negativeCount) // length of array without negative numbers
    }
...
function getOddNumbers(numbers) {
    ...
    // Modulo operator
    for (nr of numbers) {
        if (nr % 2 !== 0) { // the number is not equal to 0, it is an odd number
            oddNumbers.push(nr)
         }
    }
    return oddNumbers
...
/**
 * Return reversed text. --> counting from behind use a for loop
 * @param {*} text string
 */
