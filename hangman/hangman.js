const AVAILABLE_wordToGuess = ['qualification', 'reactor',
                         'electricity', 'coincide', 'japan', 'breeze', 'sakura']
...
function generateRandomwordToGuess() {
    let ri = Math.floor(Math.random() * AVAILABLE_wordToGuess.length)
    return AVAILABLE_wordToGuess[ri]
}
...
function handleKeyboardButtonClick(letterContainer, char) {
    ...
    if (guessWordLetterIndex !== -1) {
        ...
        if (hp === 0) {
            disableKeyboard()
            document.body.style.backgroundColor = '#f09186'
        }
    }
    letterContainer.style.pointerEvents = 'none'
}
